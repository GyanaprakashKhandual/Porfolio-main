  ...
  return (
    <section className="relative py-28 px-6 lg:px-12 bg-white dark:bg-[#0d0d0f]">
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        >
          <p className="text-xs font-medium tracking-[0.2em] uppercase text-black/40 dark:text-white/40 mb-3">What I&apos;ve built</p>
          <h2 className="text-4xl sm:text-5xl font-black tracking-tight">Core Projects</h2>
        </motion.div>
            ...
            >
              ...
              <div className="relative">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    <span className="text-xs font-medium text-black/40 dark:text-white/40 uppercase tracking-widest block mb-2">{project.tag}</span>
                    <h3 className="text-2xl font-black text-black dark:text-white">{project.name}</h3>
                  </div>
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/8 border border-black/5 dark:border-white/5">
                    ...
                    <span className="text-xs font-semibold text-black/70 dark:text-white/70">{project.users}</span>
                  </div>
                </div>

                <p className="text-sm text-black/55 dark:text-white/55 leading-relaxed mb-6">{project.description}</p>

                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tech.map(t => (
                    <span key={t} className="text-[10px] uppercase tracking-wider font-semibold px-2.5 py-1 rounded-md bg-black/5 dark:bg-white/5 text-black/50 dark:text-white/50">{t}</span>
                  ))}
                </div>

                <div className="flex gap-3">
                  <motion.a href={project.links.app} target="_blank" rel="noopener noreferrer"
                  ...
                  <motion.a href={project.links.frontend} target="_blank" rel="noopener noreferrer"
                    whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  ...
                  <motion.a href={project.links.backend} target="_blank" rel="noopener noreferrer"
                    whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    className="flex items-center gap-2 px-4 py-2.5 border border-black/10 dark:border-white/10 text-black/70 dark:text-white/70 text-sm font-medium rounded-lg hover:bg-black/3 dark:hover:bg-white/3"
                  >
        ...
        >
          <p className="text-xs font-medium tracking-[0.2em] uppercase text-black/40 dark:text-white/40 mb-2">Published on Marketplace</p>
          <h3 className="text-3xl font-black tracking-tight">VS Code Extensions</h3>
        </motion.div>
            ...
            >
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-lg text-black dark:text-white">{ext.name}</h3>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/8">
                  ...
                  <span className="text-xs font-semibold text-black/70 dark:text-white/70">{ext.installs}</span>
                </div>
              </div>
              <p className="text-sm text-black/55 dark:text-white/55 leading-relaxed mb-6">{ext.description}</p>
              <motion.a
